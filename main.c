...
int readSettings() {
    ...
    printf("readSettings: \n\tsock_port: %d, \n\tpid_path: %s, \n\tcycle_duration: %ld sec %ld nsec, \n\tdb_data_path: %s, sock_port, pid_path, cycle_duration.tv_sec, cycle_duration.tv_nsec, db_data_path);
#endif
            return 1;
}
...
void normalise_matter(Matter *item, double ambient_temperature) {
    double dE = 0.0;
    if (item->temperature > ambient_temperature) {
        dE = -((1 + item->kl)^(item->temperature - ambient_temperature) - 1);
    } else if (item->temperature < ambient_temperature) {
        dE = (1 + item->kl)^(ambient_temperature - item->temperature) - 1;
    }
    item->energy += dE;
            double dT = dE / (item->ksh * item->mass);
    if (item->temperature_pipe.length > 0) {
...
void disturb_matter(Matter *item, double heater_power, double cooler_power) {

    item->energy += heater_power;
            item->energy -= cooler_power;
}

void progControl(Prog * item) {
#ifdef MODE_DEBUG
    char *state=getStateStr(item->state);
    printf("prog: id:%d state:%s temp:%f energy:%f heater_pwr:%f cooler_pwr:%f \n", item->id, state, item->matter.temperature, item->matter.energy, item->heater.power, item->cooler.power);
#endif
    switch (item->state){
        case INIT:
        ...
        case RUN:
            //aiming to ambient
            normalise_matter(item->matter, item->ambient_temperature);
            //actuator affect
            disturb_matter(item->matter, item->heater.power, item->cooler.power);
            break;
        case OFF:
            break;
        default:
